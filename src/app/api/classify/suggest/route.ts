...
import { TRANSFER_GROUP } from '@/lib/dre'
import { NextRequest, NextResponse } from 'next/server'

const HISTORY_THRESHOLD = 0.35

/**
 * POST /api/classify/suggest
 * body: { memos: [{ fitid, memo }] }
 * Compara cada descrição com o histórico já classificado (Jaccard) e devolve
 * a melhor conta acima do threshold. Nunca aplica nada — só sugere.
 */
export async function POST(req: NextRequest) {
  ...
  // Para cada descrição única do histórico, a conta usada com mais frequência
  const memoAccountFreq = new Map<string, Map<number, { account: NonNullable<(typeof history)[0]['account']>; count: number }>>()
  for (const tx of history) {
    if (!tx.account || !tx.memo || tx.account.dreGroup === TRANSFER_GROUP) continue
  ...
  Array.from(memoAccountFreq.entries()).forEach(([memo, accountMap]) => {
    ...
    references.push({
      ...
      account: { id: best.account.id, name: best.account.name, code: best.account.code },
    })
  })
